...
const Modal = ({TotalPrice,totalQty,hideModal}) => {
    ...
    const handleCashOnDelivery=async(e)=>{
        ...
        await firestore.collection('Buyer-Personal-Info').add({
        ...
        const cartData = await firestore.collection('Cart ' + uid).get();
        for(var snap of cartData.docs){
            var data = snap.data();
            data.ID = snap.id;
            await firestore.collection('Buyer-Cart ' + uid).add(data);
            await firestore.collection('Cart ' + uid).doc(snap.id).delete();
        }
        hideModal();
        navigate('/');
